...
var EchanParam = [" 'metro' "," 'lim' "," 'min' "," 'max' "," 'env' "," 'dur' "," 'fic' "];
 

function E(index,metro,lim, min,max,env,dur,fic){
	
 this[index] new Object();
 this[index].metro = metro;
 this[index].lim = lim;
 this[index].min = min;
 this[index].max = max;
 this[index].env = env;
 ...
 this[index].fic = fic;
  ...
  E(index,metro,lim,min,max,env,dur,fic);
}	
...
function E(index, metro, lim, min, max,env,dur,fic){
	...
	for (var i = 0; arguments.length; i++){
		...
		messnamed ("E",index, EchanParam[i], arguments[i+1]);		
}
...
messnamed ("E3", "env", e);  messnamed ("E3", "dur", f);  messnamed ("E3", "buffer", g); messnamed ("E3s", "s", 1); 
}
function E3s(y, x) {
